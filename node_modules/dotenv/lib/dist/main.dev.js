...
var crypto = require('crypto'); // Array of tips to display randomly


var TIPS = ['◈ encrypted .env [www.dotenvx.com]', '◈ secrets for agents [www.dotenvx.com]', '⌁ auth for agents [www.vestauth.com]', '⌘ custom filepath { path: \'/custom/path/.env\' }', '⌘ enable debugging { debug: true }', '⌘ override existing { override: true }', '⌘ suppress logs { quiet: true }', '⌘ multiple files { path: [\'.env.local\', \'.env\'] }']; // Get a random tip from the tips array
...
function _parseVault(options) {
  ...
  var result = DotenvModule.configDotenv(options);
...
function _configVault(options) {
  var debug = parseBoolean(process.env.DOTENV_CONFIG_DEBUG || options && options.debug);
  var quiet = parseBoolean(process.env.DOTENV_CONFIG_QUIET || options && options.quiet);
...
function configDotenv(options) {
  ...
  var debug = parseBoolean(processEnv.DOTENV_CONFIG_DEBUG || options && options.debug);
  var quiet = parseBoolean(processEnv.DOTENV_CONFIG_QUIET || options && options.quiet);
  ...
  var populated = DotenvModule.populate(processEnv, parsedAll, options); // handle user settings DOTENV_CONFIG_ options inside .env file(s)

  debug = parseBoolean(processEnv.DOTENV_CONFIG_DEBUG || debug);
  quiet = parseBoolean(processEnv.DOTENV_CONFIG_QUIET || quiet);

  if (debug || !quiet) {
    var keysCount = Object.keys(populated).length;
    var shortPaths = [];
    ...
    try {
      for (var _iterator3 = optionPaths[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        var filePath = _step3.value;

        try {
          var relative = path.relative(process.cwd(), filePath);
    ...
    _log("injected env (".concat(keysCount, ") from ").concat(shortPaths.join(','), " ").concat(dim("// tip: ".concat(_getRandomTip()))));
  }
...
function config(options) {
  ...
  if (!vaultPath) {
    _warn("you set DOTENV_KEY but you are missing a .env.vault file at ".concat(vaultPath));
