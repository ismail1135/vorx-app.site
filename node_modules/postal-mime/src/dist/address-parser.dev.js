...
var _decodeStrings = require("./decode-strings.js");
...
function _handleAddress(tokens, depth) {
  ...
  var i;
  var len;
  var insideQuotes = false; // Track if we're currently inside a quoted string
  ...
  if (isGroup) {
    ...
    data.text = data.text.join(' '); // Parse group members, but flatten any nested groups (RFC 5322 doesn't allow nesting)

    var groupMembers = [];

    if (data.group.length) {
      var parsedGroup = addressParser(data.group.join(','), {
        _depth: depth + 1
      }); // Flatten: if any member is itself a group, extract its members into the sequence

      parsedGroup.forEach(function (member) {
        if (member.group) {
    ...
    addresses.push({
      name: (0, _decodeStrings.decodeWords)(data.text || address && address.name),
      group: groupMembers
    });
  } else {
    ...
    if (!data.address && /^=\?[^=]+?=$/.test(data.text.trim())) {
      ...
      var decodedText = (0, _decodeStrings.decodeWords)(data.text); // Security: only re-parse if decoded text contains angle-bracket addresses.
      ...
      if (/<[^<>]+@[^<>]+>/.test(decodedText)) {
        var parsedSubAddresses = addressParser(decodedText);
    ...
    address = {
      ...
      name: (0, _decodeStrings.decodeWords)(data.text || data.address || '')
    };
...
function addressParser(str, options) {
  options = options || {};
  var depth = options._depth || 0; // Prevent stack overflow from deeply nested groups (DoS protection)
