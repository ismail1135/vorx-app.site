...
test('merge()', function (t) {
  ...
  t.test('with overflow objects (from arrayLimit)', function (st) {
    ...
    st.test('with strictMerge, wraps object and primitive in array', function (s2t) {
      ...
      s2t.deepEqual(merged, [{
        foo: 'bar'
      }, 'baz'], 'wraps in array with strictMerge');
      s2t.end();
    });
    ...
    st.test('merges overflow object into primitive with plainObjects', function (s2t) {
      ...
      s2t.deepEqual(merged, {
        ...
        1: 'a',
        2: 'b'
      }, 'creates null-proto object with primitive at 0');
      s2t.end();
    });
    ...
    st.test('merges primitive into array that exceeds arrayLimit', function (s2t) {
      ...
      s2t.deepEqual(merged, {
        ...
        2: 'c',
        3: 'd'
      }, 'converts to overflow object with primitive appended');
      s2t.end();
    });
    st.test('merges array into primitive that exceeds arrayLimit', function (s2t) {
      var merged = utils.merge('a', ['b', 'c'], {
        arrayLimit: 1
      });
      s2t.ok(utils.isOverflow(merged), 'result is marked as overflow');
      s2t.deepEqual(merged, {
...
test('combine()', function (t) {
  ...
  t.test('with arrayLimit', function (st) {
    ...
    st.test('with arrayLimit 0 converts single element to object', function (s2t) {
      ...
      s2t.deepEqual(combined, {
        0: 'a'
      }, 'converts to object when count exceeds limit');
      s2t.notOk(Array.isArray(combined), 'result is not an array');
      s2t.end();
    });
  ...
  t.test('with existing overflow object', function (st) {
    ...
    st.test('does not treat plain object with numeric keys as overflow', function (s2t) {
      ...
      s2t.notOk(utils.isOverflow(plainObj), 'plain object is not marked as overflow'); // combine treats this as a regular value, not an overflow object to append to

      var combined = utils.combine(plainObj, 'c', 10, false);
      s2t.deepEqual(combined, [{
        0: 'a',
        1: 'b'
      }, 'c'], 'concatenates as regular values');
      s2t.end();
    });
    st.end();
  });
...
test('decode', function (t) {
  ...
  t.equal(utils.decode('name%2Eobj%2Efoo', null, 'iso-8859-1'), 'name.obj.foo', 'decodes a string in iso-8859-1');
  t.end();
});
test('encode', function (t) {
  ...
  t.equal(utils.encode({
    toString: function toString() {
      return '(abc)';
    }
  }), '%28abc%29', 'toStrings and encodes parentheses');
  t.equal(utils.encode('abc 123 💩', null, 'iso-8859-1'), 'abc%20123%20%26%2355357%3B%26%2356489%3B', 'encodes in iso-8859-1');
  var longString = '';
  var expectedString = '';
  ...
  t.equal(utils.encode('\x28\x29', null, null, null, 'RFC1738'), '()', 'does not encode parens in RFC1738'); // todo RFC1738 format

  t.equal(utils.encode('Āက豈'), '%C4%80%E1%80%80%EF%A4%80', 'encodes multibyte chars');
  t.equal(utils.encode("\uD83D \uDCA9"), '%F0%9F%90%A0%F0%BA%90%80', 'encodes lone surrogates');
  t.end();
});
