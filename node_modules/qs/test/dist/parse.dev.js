...
test('parse()', function (t) {
  ...
  t.test('ignores prototype keys when depth = 0 and allowPrototypes is false', function (st) {
    st.deepEqual(qs.parse('toString=foo', {
      depth: 0
    }), {});
    st.deepEqual(qs.parse('hasOwnProperty=bar', {
      depth: 0
    }), {});
    st.deepEqual(qs.parse('toString=foo&a=b', {
      depth: 0
    }), {
      a: 'b'
    });
  ...
  t.test('strictMerge wraps object and primitive into an array', function (st) {
    ...
    st.deepEqual(qs.parse('a=d&a[b]=c'), {
      a: ['d', {
        b: 'c'
      }]
  ...
  t.test('handles a custom decoder returning `null`, with a string key of `null`', function (st) {
    st.deepEqual(qs.parse('null=1&ToNull=2', {
      decoder: function decoder(str, defaultDecoder, charset) {
  ...
  t.test('interpretNumericEntities with comma:true and iso charset does not crash', function (st) {
    st.deepEqual(qs.parse('b&a[]=1,' + urlEncodedNumSmiley, {
      comma: true,
      charset: 'iso-8859-1',
      interpretNumericEntities: true
    }), {
  ...
  t.test('allows for decoding keys and values differently', function (st) {
    ...
    var noopDecoder = function noopDecoder() {
      return 'x';
    };

    noopDecoder();
    st['throws'](function () {
      decoder('x', noopDecoder, 'utf-8', 'unknown');
    }, 'this should never happen! type: unknown', 'decoder throws for unexpected type');
    st.end();
  });
  t.test('parameter limit tests', function (st) {
    ...
    st.test('throws error when throwOnLimitExceeded is present but not boolean', function (sst) {
      ...
      }, new TypeError('`throwOnLimitExceeded` option must be a boolean'), 'throws error when throwOnLimitExceeded is present and not boolean');
      sst.end();
    });
    st.test('throws error when parameter limit exceeded', function (sst) {
      ...
      }, new RangeError('Parameter limit exceeded. Only 3 parameters allowed.'), 'throws error when parameter limit is exceeded');
      sst['throws'](function () {
        qs.parse('a=1&b=2', {
          parameterLimit: 1,
          throwOnLimitExceeded: true
        });
      }, new RangeError('Parameter limit exceeded. Only 1 parameter allowed.'), 'throws error with singular "parameter" when parameterLimit is 1');
      sst.end();
    });
    ...
    st.test('allows unlimited parameters when parameterLimit is Infinity and throwOnLimitExceeded is true', function (sst) {
      ...
      sst.deepEqual(result, {
        ...
        e: '5',
        f: '6'
      }, 'parses all parameters without truncation or throwing');
      sst.end();
    });
    st.end();
  });
  t.test('array limit tests', function (st) {
    ...
    st.test('throws error when throwOnLimitExceeded is present but not boolean for array limit', function (sst) {
      ...
      }, new TypeError('`throwOnLimitExceeded` option must be a boolean'), 'throws error when throwOnLimitExceeded is present and not boolean for array limit');
      sst.end();
    });
    st.test('throws error when array limit exceeded', function (sst) {
      ...
      }, new RangeError('Array limit exceeded. Only 3 elements allowed in an array.'), 'throws error when array limit is exceeded');
      sst.end();
    });
    ...
    st.test('throws error when indexed notation exceeds arrayLimit with throwOnLimitExceeded', function (sst) {
      ...
      }, new RangeError('Array limit exceeded. Only 1000 elements allowed in an array.'), 'throws error for a single index exceeding arrayLimit');
      sst['throws'](function () {
        qs.parse('a[0]=1&a[1]=2&a[2]=3&a[10]=4', {
          arrayLimit: 6,
          throwOnLimitExceeded: true,
          allowSparse: true
        });
      }, new RangeError('Array limit exceeded. Only 6 elements allowed in an array.'), 'throws error when a sparse index exceeds arrayLimit');
      sst['throws'](function () {
        qs.parse('a[2]=b', {
          arrayLimit: 1,
          throwOnLimitExceeded: true
        });
      }, new RangeError('Array limit exceeded. Only 1 element allowed in an array.'), 'throws error with singular "element" when arrayLimit is 1');
      sst.end();
    });
    st.test('does not throw for indexed notation within arrayLimit with throwOnLimitExceeded', function (sst) {
      var result = qs.parse('a[4]=b', {
        arrayLimit: 5,
        throwOnLimitExceeded: true,
        allowSparse: true
      });
    ...
    st.test('silently converts to object for indexed notation exceeding arrayLimit without throwOnLimitExceeded', function (sst) {
      ...
      }, 'converts to object without throwing');
      sst.end();
    });
    st.test('throws when duplicate bracket keys exceed arrayLimit with throwOnLimitExceeded', function (sst) {
      ...
      }, new RangeError('Array limit exceeded. Only 5 elements allowed in an array.'), 'throws error when duplicate bracket notation exceeds array limit');
      sst.end();
    });
    st.end();
  });
...
test('`duplicates` option', function (t) {
  ...
  t.test('bracket notation always combines regardless of duplicates', function (st) {
    st.deepEqual(qs.parse('a=1&a=2&b[]=1&b[]=2', {
      duplicates: 'last'
    }), {
      a: '2',
      b: ['1', '2']
    }, 'duplicates last: unbracketed takes last, bracketed combines');
    st.deepEqual(qs.parse('b[]=1&b[]=2', {
      duplicates: 'last'
    }), {
      b: ['1', '2']
...
test('qs strictDepth option - non-throw cases', function (t) {
  t.test('when depth is 0 and strictDepth true, do not throw', function (st) {
    ...
    }, RangeError, 'does not throw RangeError');
    st.end();
  });
...
test('arrayLimit boundary conditions', function (t) {
  ...
  t.test('one over the limit converts to object', function (st) {
    // 4 elements exceeds limit of 3
    var result = qs.parse('a[]=1&a[]=2&a[]=3&a[]=4', {
      arrayLimit: 3
    });
...
test('comma + arrayLimit', function (t) {
  ...
  t.test('comma-separated values exceeding arrayLimit with throwOnLimitExceeded throws', function (st) {
    ...
    }, new RangeError('Array limit exceeded. Only 3 elements allowed in an array.'), 'throws error when comma-split exceeds array limit');
    st['throws'](function () {
      qs.parse('a=1,2,3', {
        comma: true,
        arrayLimit: 1,
...
test('mixed array and object notation', function (t) {
  ...
  t.test('array index with object key - under limit', function (st) {
    st.deepEqual(qs.parse('a[0]=b&a[c]=d'), {
      a: {
        0: 'b',
        c: 'd'
      }
  ...
  t.test('mixed notation produces consistent results when arrayLimit is exceeded', function (st) {
    ...
    st.deepEqual(qs.parse('a[]=b&a[1]=c&a=d', {
      arrayLimit: -1
    }), expected, 'arrayLimit -1');
    st.deepEqual(qs.parse('a[]=b&a[1]=c&a=d', {
      arrayLimit: 0
  ...
  t.test('uses existing array length for currentArrayLength when parsing object input with bracket keys', function (st) {
    ...
    arr.a = ['z', 'w'];
    input['a[]'] = arr;
    st.deepEqual(qs.parse(input), {
      a: ['x', 'y']
    }, 'parses object input with bracket keys using existing array values');
    st.end();
  });
  t.test('throws with singular message when object input bracket key exceeds arrayLimit of 1', function (st) {
    ...
    }, new RangeError('Array limit exceeded. Only 1 element allowed in an array.'), 'throws singular error for object input exceeding arrayLimit 1');
    st.end();
  });
  t.end();
});